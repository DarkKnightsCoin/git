...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("DarkKnightsCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #DarkKnightsCoinTEST2\r");
            Send(hSocket, "WHO #DarkKnightsCoinTEST2\r");
        } else {
            // randomly join #DarkKnightsCoin00-#DarkKnightsCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #DarkKnightsCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #DarkKnightsCoin%02d\r", channel_number).c_str());
        }
