...
#else
    const char* pszModule = "DarkKnightsCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DarkKnightsCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DarkKnightsCoin
    // Mac: ~/Library/Application Support/DarkKnightsCoin
    // Unix: ~/.DarkKnightsCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DarkKnightsCoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "DarkKnightsCoin";
#else
    // Unix
    return pathRet / ".DarkKnightsCoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "DarkKnightsCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "DarkKnightsCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong DarkKnightsCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("DarkKnightsCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
